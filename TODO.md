...
- [ ] **SymSpell dégrade la transcription** — Le spellcheck remplace des mots corrects par des mots fréquents hors contexte (ex: "un gros lag" → "un très la", "Tu as" → "Du pas"). Le dictionnaire de fréquences n'a aucune notion de contexte. Nécessite soit un modèle de langage (KenLM/n-gram) pour valider les corrections, soit une approche plus conservative (ne corriger que les mots absents du dictionnaire). Pistes de recherche dans MEMORY (`pipeline-improvements.md`).
