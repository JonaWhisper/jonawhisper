...
- [x] **Spectre plat/grisé pendant l'enregistrement (intermittent)** — Résolu. Cause : `try_lock()` sur le `Mutex<Vec<f32>>` du spectre pouvait échouer quand l'emitter 30fps tenait le verrou au même moment → updates FFT perdues silencieusement. Fix : remplacé le `Mutex` par `AtomicSpectrum` (12 `AtomicU32` avec f32-to-bits) — zéro contention entre le callback cpal temps-réel et l'emitter. Smoothing déplacé du callback audio (où il bloquait sous mutex) vers l'emitter thread. Diagnostic conservé.
