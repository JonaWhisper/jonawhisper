...
- [ ] **Setup wizard ne s'affiche pas après relancement (permissions déjà accordées)** — Le setup se base uniquement sur `check_permissions().all_granted()` (`lib.rs:342`). Si l'utilisateur accorde Input Monitoring au step 1, macOS demande de quitter/relancer, et au relancement les permissions sont OK → le setup ne s'affiche plus et le step 2 (langue, modèle, raccourci) est sauté. Fix : persister un flag `setup_completed` dans Preferences, et afficher le setup tant que ce flag est false, indépendamment des permissions. Le flag passe à true uniquement quand l'utilisateur clique "Démarrer" à la fin du step 2.
- [ ] **Check Input Monitoring trop permissif** — Le check de permission (`macos.rs:81`) utilisait le masque `kCGEventFlagsChanged` uniquement, ce qui pouvait retourner Granted alors que les KEY_DOWN/KEY_UP étaient bloqués. Corrigé sur `feat/memory-diagnostics` (masque élargi à keyDown+keyUp+flagsChanged). À merger.
