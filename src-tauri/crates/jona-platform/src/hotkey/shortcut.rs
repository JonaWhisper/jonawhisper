...
use super::keys;
use super::{
    MASK_ALTERNATE, MASK_COMMAND, MASK_CONTROL, MASK_SHIFT,
};
...
/// One label per held modifier. macOS concatenates single glyphs, Windows
/// spells the names out, so the caller joins rather than the callee.
use super::labels::{MODIFIER_JOIN, key_code_label, modifier_labels, modifier_only_label};

fn modifier_symbols(flags: u64) -> String {
    modifier_labels(flags).join(MODIFIER_JOIN)
}
