...
const POST_LABELS: &[&str] = &[
    "", ".", ",", "?", "\u{ff1f}", "\u{ff0c}", "\u{3002}", "\u{3001}", "\u{30fb}", "\u{0964}", "\u{061f}", "\u{060c}", ";", "\u{1362}", "\u{1363}", "\u{1367}",
];

/// Pre-token punctuation labels (2 classes).
const PRE_LABELS: &[&str] = &["", "\u{00bf}"];
...
impl PcsContext {
    ...
    pub fn load(model_path: &Path, model_id: &str) -> Result<Self, String> {
        ...
        let session = jona_engines::ort_session::build_session(n_threads)?
...
fn reconstruct_text(
    ...
    for (token_idx, token) in tokens.iter().enumerate() {
        let has_space_prefix = token.starts_with('\u{2581}');

        if has_space_prefix && !chars.is_empty() {
            chars.push(' ');
        }

        let char_start = if has_space_prefix { '\u{2581}'.len_utf8() } else { 0 };
        ...
        .filter(|c| {
            !matches!(
                ...
                '.' | ',' | '?' | ':' | '-' | ';' | '!' | '\u{3002}' | '\u{ff0c}' | '\u{ff1f}'
                    | '\u{3001}' | '\u{30fb}' | '\u{0964}' | '\u{061f}' | '\u{060c}' | '\u{1362}' | '\u{1363}' | '\u{1367}' | '\u{00bf}'
            )
        })
        .collect()
}
